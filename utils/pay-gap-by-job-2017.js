function PayGapByJob2017() {
  ...
  this.draw = function() {
    ...
    for (i = 0; i < this.data.getRowCount(); i++) {
      ...
      let y = map(payGap[i], payGapMin, payGapMax, height - this.pad, 0 + this.pad);
      let size = map(numJobs[i], numJobsMin, numJobsMax, 0, 50);

      ellipse(x, y, size);

      if ((mouseX > x - size && mouseX < x + size) && (mouseY > y - size && mouseY < y + size)) {
        toolTip({'Paygap' : y, 'Female Proportion' : x, 'Number of Jobs' : size});
      }
